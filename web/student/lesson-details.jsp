                    ...
                    <div class="layout-specing">
                        <div class="row">
                            <div class="col-12">
                                ...
                                <span> > </span>
                                <a href="/my-classes/lesson?lessonId=${lesson.lessonId}&classId=${classroom.id}&subjectId=${lesson.subject.id}">${lesson.name}</a>
                            </div>
                        </div>
                        <div class="row">
                            <div class="d-flex justify-content-between align-items-center w-100">
                                <div class="d-flex w-75 justify-content-end">
                                    <div class="form-floating w-100 me-2">
                                        <select class="form-select" id="floatingClass" aria-label="Floating label select example" onchange="redirectToDetails(this)">
                                            <c:forEach var="c" items="${classList}">
                                                <option value="${c.id},${c.subject.id}" ${classId == c.id ? 'selected' : ''}>${c.title}</option>
                                            </c:forEach>
                                        </select>
                                        <label for="floatingSelect">Current class</label>
                                    </div>
                                    <div class="form-floating w-100 ms-2">
                                        <select class="form-select" id="floatingChapter" aria-label="Floating label select example" onchange='redirectToChapter(this)'>
                                            <c:forEach var="l" items="${lessonList}">
                                                <option value="${classroom.id},${l.subject.id},${l.lessonId}" ${lessonId == l.lessonId ? 'selected' : ''}>${l.name}</option>
                                            </c:forEach>
                                        </select>
                        ...
                        <div class="row">
                            <div class="col-12">
                                ...
                                <c:forEach var="material" items="${materials}">
                                    <a href="../material/${material.materialName}" download class="btn btn-primary">${material.materialName}</a>
                                </c:forEach>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
                                <p>Flashcard</p>
                                <a href="/my-classes/flashcard" class="btn btn-primary">Flashcard</a>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
