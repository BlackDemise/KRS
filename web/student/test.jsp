...
<html lang="en">
    ...
    <body>
        ...
        <div class="container-fluid" style="margin-top: 150px;">
            ...
            <form action="/student/test/score" method="post">
                <input type="hidden" name="examId" value="${examId}"/>
                <input type="hidden" name="classId" value="${classId}"/>
                <c:forEach var="q" items="${questions}">
                    <div class="row mx-3 my-5 py-3 px-3" style="border:1px solid black" id="question-area-${q.id}">
                        ...
                        <c:forEach var="a" items="${answers}">
                            <c:choose>
                                <c:when test="${q.id == a.question.id}">
                                    <div id="answer-${a.answerId}" data-correct="${a.correct}" style="width:48%;border:2px solid gray;border-radius:7px;text-align: center;margin:10px;padding:10px" class="answer d-flex justify-content-center align-items-center" onclick="selectAnswer(${q.id}, ${a.answerId})">
                                        <p>${a.answer}</p>
                                        <input type="radio" name="question-${q.id}" value="${a.answerId}"/>
                                    </div>
                                </c:when>
                            </c:choose>
                        </c:forEach>
                ...
                <div class="row" style="margin-top:10px">
                    <div class="col-12 d-flex justify-content-center">
                        <button class="btn btn-light mx-2" type="button" onclick="resetAnswers()">Reset</button>
        ...
        <script>
                            function selectAnswer(questionId, answerId) {
                                ...
                                const questionArea = document.getElementById(`question-area-${questionId}`);
                                const answers = questionArea.getElementsByClassName('answer');
                                for (let answer of answers) {
                                    answer.style.borderColor = 'gray';
                                }
                                // Select the clicked answer
                                const selectedAnswer = document.getElementById(`answer-${answerId}`);
                                selectedAnswer.style.borderColor = 'blue';
                                // Mark the corresponding radio button as checked
                                const radioButton = selectedAnswer.querySelector('input[type="radio"]');
